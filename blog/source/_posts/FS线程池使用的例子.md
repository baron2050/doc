...
static void *SWITCH_THREAD_FUNC read_frame_thread(switch_thread_t *thread, void *obj){
    ...
    while((pvt) && (pvt->sock) && (pvt->local_port > 0)){
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "read_frame  number = %ld !\n",len);
        if(len > 0){
            pvt->udp_frame.datalen = len;
            pvt->udp_frame.data = buf;
            pvt->udp_frame.buflen = len;
            //pvt->udp_frame.samples= len / 2
            //if (switch_core_media_write_frame(pvt->xsession, &pvt->udp_frame,SWITCH_IO_FLAG_QUEUED,pvt->packet_num,SWITCH_MEDIA_TYPE_AUDIO) != SWITCH_STATUS_SUCCESS) {
            //    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "frame send error!\n");
		    //}
        }
        switch_sleep(20);
    }
    ...
    case SWITCH_ABC_TYPE_READ_REPLACE:
        {
            ...
            if ((frame = switch_core_media_bug_get_read_replace_frame(bug)))
            {
                ...
                if ((pvt) && (pvt->sock))
                {
                    ...
                    //
                    //frame->data = bufs;
                    //switch_core_session_write_frame(pvt->xsession, frame,SWITCH_IO_FLAG_QUEUED,pvt->packet_num);
...
SWITCH_STANDARD_API(uuid_frame_write_function)
{
    ...
    if (strcmp(argv[1], "on") == 0)
    {
        ...
        // pvt->remote_port = switch_sockaddr_get_port(pvt->r_sockaddr);
            switch_core_session_get_read_impl(xsession, &read_impl);
